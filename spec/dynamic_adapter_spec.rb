...
describe "ClientSync" do
  it_behaves_like "SharedRhosyncHelper", :rhosync_data => true do
    ...
    it "should update object using dynamic adapter" do
      data = {'id' => 2} 
      stub_request(:post, "http://test.rhosync.com/rhosync/update").with(:headers => {'Content-Type' => 'application/json'}).to_return(:body => {:id => 5})
      da = DynamicAdapter.new(@s2,nil,'http://test.rhosync.com')
    ...
    it "should delete object using dynamic adapter" do
      data = {'id' => 2}
      stub_request(:post, "http://test.rhosync.com/rhosync/delete").with(:headers => {'Content-Type' => 'application/json'}).to_return(:body => {:id => 5})
      da = DynamicAdapter.new(@s2,nil,'http://test.rhosync.com')
