...
# Stub generated by 'rhosync source product' 
class Product < SourceAdapter
  ...
  # Lots of unneeded stuff omitted...
end
...
describe "SourceAdapter" do
  it_behaves_like "SharedRhosyncHelper", :rhosync_data => true do
    ...
    describe "SourceAdapter methods" do
      ...
      context "Instance of derived SourceAdapter class" do
        before(:each) do
          @s.name = 'Product'          
        end

        it "should be valid when created with only source parameter" do
          @sa = Product.new(@s)
          @sa.class.name.should == @s.name          
        end

        it "should be valid when created with two parameters and issue log warning" do
          @sa = Product.new(@s, "some credentials go here")
          @sa.class.name.should == @s.name          
        end
      end
